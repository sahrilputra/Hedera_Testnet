...
const createAccTransferHbar = async () => {
    ...
    const sendHbar = await new TransferTransaction()
        .addHbarTransfer(myAccountId, Hbar.fromTinybars(-100)) //Sending
        .addHbarTransfer(newAccountId, Hbar.fromTinybars(100)) //Recieve
        .execute(client);

    const transactionReceipt = await sendHbar.getReceipt(client);
    console.log("Status Of Transfer Transsaction : " + transactionReceipt.status.toString());
